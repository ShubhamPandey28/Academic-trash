...
mean = list(map(int,input().split(" ")))
print("enter covariance matrix")
cov1 = list(map(int,input().split(" ")))
cov2 = list(map(int,input().split(" ")))
covx = [cov1,cov2]
X=[]
Y=[]
for i in range(1000):
    x, y = np.random.multivariate_normal(mean,covx,1).T
    X=X+list(x)
    Y=Y+list(y)
...
error0=(sum(E0)**0.5)/len(E0) #error when both vectors are used
error1=(sum(E1)**0.5)/len(E1) #error when vector 1 is used
error2=(sum(E2)**0.5)/len(E2) #error when vector 2 is used
