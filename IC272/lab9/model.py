...
class linear_regretion(object):
    ...
    def predict(self,x):
        x = np.array(x)
        return np.dot(x,self.w)+np.random.norm(0,1)
...
class polynomialRegression:
    ...
    def fit(self,X,Y,degree):
        self.x_train, self.x_test,self.y_train, self.y_test = tts(X,Y,test_size=0.5,random_state=42,shuffle=True)  
    ...
    def predict(self,x):
        return np.dot(x,self.w)+np.random.normal(0,1)
